...
public class SpringGraphManager : MonoBehaviour
{
    [SerializeField]
    ...
    List<SpringNode> _nodes;
    List<Edge<SpringNode>> _edges;
    UndirecteGraph<SpringNode, Edge<SpringNode>> _undirectedGraph;
    ...
    void RayCast()
    {
        ...
        if (Input.GetMouseButton(0) && Physics.Raycast(ray, out hit))
        {
            ...
            if (objectHit.tag == "Node")
            {
                ...
                foreach (var vertex in _undirectedGraph.GetNeighbourVertices(objectHit.GetComponent<SpringNode>()))
                {
                    vertex.GetComponent<MeshRenderer>().material = _materials[1];
                }

                Dijkstra<SpringNode, Edge<SpringNode>> dijkstra = new Dijkstra<SpringNode, Edge<SpringNode>>(_undirectedGraph);
